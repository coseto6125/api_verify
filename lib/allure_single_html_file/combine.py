# -*- coding: utf-8 -*-
# @Author: E-NoR
# @Date:   2022-03-17 17:18:51
# @Last Modified by:   E-NoR
# @Last Modified time: 2022-12-20 18:56:06
#! /usr/bin/env python3
"""
...
def combine_allure(folder, filename, debug=False):
    """
    Read all files,
    create server.js,
    then run server.js,
    ...
    if debug:
        print(f"> Folder to process is {folder}")
        print("> Checking for folder contents")

    files_should_be = ["index.html", "app.js", "styles.css"]

    for file in files_should_be:
        if not os.path.exists(f"{folder}{sep}{file}"):
            raise RuntimeError(f"ERROR: File {folder}{sep}{file} doesnt exists, but it should!")
            ...
            if folder_url and folder_url != folder:
                for file in files:
                    ...
                    if ext not in allowed_extensions:
                        print(
                            f"WARNING: Unsupported extension: {ext} (file: {path}{sep}{file}) skipping (supported are: {allowed_extensions}"
                        )
                        continue
                    ...
                    if ext in base64_extensions:
                        ...
                        with open(path + sep + file, "r", encoding="utf-8-sig") as f:
                            content = f.read()

                    data.append(
                        {"url": file_url, "mime": mime, "content": content, "base64": (ext in base64_extensions)}
                    )
    if debug:
    ...
    copyfile(f"{cur_dir}{sep}sinon-9.2.4.js", f"{folder}{sep}sinon-9.2.4.js")
    if debug:
    ...
    with open(filename, "w", encoding="utf8") as f:
        f.write(str(soup))
    if debug:
        print(f"> Saving result as {filename}")

    size = os.path.getsize(filename)
    print(f"報告建立完畢。檔案大小: {round(size/1048576,2)}MB")
